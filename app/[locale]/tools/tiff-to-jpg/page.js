...
export default function TiffToJpgPage() {
  ...
  const t = useTranslations("tools.tiffToJpg");
  ...
  const processFile = useCallback(
    (file) => {
      ...
      const isTiff = file.type === "image/tiff" || file.name.toLowerCase().match(/\.tif(f)?$/);
      if (!isTiff) {
        setError(t("errorTiffOnly"));
        setTiffFile(null);
        return;
      }
      setError("");
      ...
      setTiffFile(file);
      setTiffUrl(URL.createObjectURL(file));
      setCurrentStep(2);
    },
    [t]
  );
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        img.onload = resolve;
        img.onerror = () => reject(new Error("TIFF_NOT_SUPPORTED"));
        img.src = tiffUrl;
      });
      ...
      setError(err?.message === "TIFF_NOT_SUPPORTED" ? t("errorTryChromeSafari") : t("errorConversionFailed"));
    } finally {
  ...
  const handleDownload = useCallback(() => {
    ...
    const base = tiffFile?.name?.replace(/\.tif(f)?$/i, "") || "image";
    const a = document.createElement("a");
    a.href = resultUrl;
    a.download = `${base}.jpg`;
    a.click();
  }, [resultBlob, resultUrl, tiffFile]);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
              ...
              {!tiffFile ? (
                ...
                <div className="flex h-[200px] w-full items-center gap-4 rounded-xl border-2 border-dashed border-sky-500/50 bg-slate-900/60 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-slate-100">{tiffFile.name}</p>
                    <p className="mt-1 text-xs text-slate-400">{formatBytes(tiffFile.size)}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.tiffToJpg" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="tiff-to-jpg" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.tiffToJpg" />
        </div>
      </main>
    </div>
  );
