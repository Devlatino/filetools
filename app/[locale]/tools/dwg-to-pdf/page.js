...
export default function DwgToPdfPage() {
  ...
  const t = useTranslations("tools.dwgToPdf");
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch("/api/dwg-to-pdf", {
        method: "POST",
        body: formData,
      });
      ...
      a.href = url;
      a.download = file.name.replace(/\.dwg$/i, ".pdf");
      a.click();
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.dwgToPdf" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="dwg-to-pdf" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.dwgToPdf" />
        </div>
      </main>
    </div>
  );
