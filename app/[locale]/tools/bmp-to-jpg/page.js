...
export default function BmpToJpgPage() {
  ...
  const t = useTranslations("tools.bmpToJpg");
  ...
  const handleDownload = useCallback(() => {
    ...
    const base = originalFile?.name?.replace(/\.bmp$/i, "") || "image";
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.bmpToJpg" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="bmp-to-jpg" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.bmpToJpg" />
        </div>
      </main>
    </div>
  );
