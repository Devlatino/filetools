...
export default function GifToMp4Page() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      await ffmpeg.writeFile(INPUT_NAME, await fetchFile(gifFile));
      await ffmpeg.exec([
        "-i",
        INPUT_NAME,
        "-movflags",
  ...
  }, [gifFile, loadFfmpeg, t]);

  const handleDownload = useCallback(() => {
    if (!resultBlob || !resultUrl) return;
    const base = gifFile?.name?.replace(/\.gif$/i, "") || "video";
    const a = document.createElement("a");
    a.href = resultUrl;
    a.download = `${base}.mp4`;
    a.click();
  }, [resultBlob, resultUrl, gifFile]);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          <section className="space-y-6">
              ...
              {!gifFile ? (
                ...
                <div className="flex h-[200px] w-full items-center gap-4 rounded-xl border-2 border-dashed border-sky-500/50 bg-slate-900/60 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-slate-100">{gifFile.name}</p>
                    <p className="mt-1 text-xs text-slate-400">{formatBytes(gifFile.size)}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            {gifFile && !resultBlob && !isLoadingFfmpeg && (
              <button
                type="button"
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.gifToMp4" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="gif-to-mp4" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.gifToMp4" />
        </div>
      </main>
    </div>
  );
