...
import { Upload, Loader2, Check, Download, Archive, X } from "lucide-react";
...
export default function CreateZipPage() {
  ...
  const removeFile = useCallback((index) => {
    setFiles((prev) => {
      ...
      if (next.length === 0) setCurrentStep(1);
      return next;
    });
    setResultBlob(null);
  }, []);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
            ...
            <div className="w-full">
              ...
              >
                ...
                <Upload
                  size={40}
                  strokeWidth={1.5}
                  className={isDragOver ? "text-emerald-400" : "text-emerald-500/80"}
                />
                <p className={`text-center text-sm font-medium ${isDragOver ? "text-emerald-200" : "text-slate-300"}`}>
                  {isDragOver ? t("releaseToUpload") : tTool("dropZone")}
                </p>
                <p className="text-center text-xs text-slate-500">{t("addMoreHint")}</p>
              </label>
            </div>
            ...
            {files.length > 0 && (
              <>
                <p className="text-sm text-slate-400">
                  {t("filesCount", { count: files.length })} · {t("totalSize", { size: formatBytes(totalSize) })}
                </p>
                <ul className="max-h-48 space-y-2 overflow-y-auto rounded-xl border border-white/10 bg-slate-900/60 p-3">
                  {files.map((file, index) => (
                    <li
                      key={`${file.name}-${index}`}
                      className="flex items-center justify-between gap-2 rounded-lg bg-slate-800/80 px-3 py-2 text-sm"
                    >
                      <span className="min-w-0 truncate text-slate-200">{file.name}</span>
                      <span className="shrink-0 text-xs text-slate-500">{formatBytes(file.size)}</span>
                      <button
                        type="button"
                        onClick={() => removeFile(index)}
                        className="shrink-0 rounded p-1 text-slate-400 hover:bg-rose-500/20 hover:text-rose-400"
                ...
                {!resultBlob && (
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-3 rounded-xl bg-emerald-500 py-4 text-base font-semibold text-slate-950 shadow-lg shadow-emerald-500/25 transition hover:bg-emerald-400 hover:shadow-emerald-400/30 disabled:cursor-not-allowed disabled:opacity-70"
                  >
                    {isCreating ? (
                      <>
                      ...
                      <>
                        <Archive size={22} strokeWidth={2} className="shrink-0" />
                        <span>{t("createZipButton")}</span>
                      </>
                    )}
                  </button>
                )}
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.createZip" />
          </aside>
        </div>
        ...
        <div className="mt-10">
          <FaqSection namespace="tools.createZip" />
        </div>
      </main>
    </div>
  );
