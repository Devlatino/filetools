...
import { Upload, Loader2, Check, FileOutput } from "lucide-react";
...
export default function PdfToPngPage() {
  ...
  const t = useTranslations("tools.pdfToPng");
  ...
  const handleDownloadOne = useCallback((item) => {
    ...
    a.download = `${base}-page-${item.pageIndex}.png`;
    a.click();
  }, [pdfFile]);
  ...
  const handleDownloadZip = useCallback(async () => {
    ...
    results.forEach((item) => {
      zip.file(`${base}-page-${item.pageIndex}.png`, item.blob);
    });
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.pdfToPng" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="pdf-to-png" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.pdfToPng" />
        </div>
      </main>
    </div>
  );
