...
import { Upload, Loader2, Check, Download, VolumeX } from "lucide-react";
...
export default function MuteVideoPage() {
  ...
  const t = useTranslations("tools.muteVideo");
  ...
  }, [videoFile, loadFfmpeg, t]);

  const handleDownload = useCallback(() => {
    if (!resultBlob || !resultUrl) return;
    const ext = getExt(videoFile);
    const base = videoFile?.name?.replace(/\.(mp4|webm|mov)$/i, "") || "muted";
    const a = document.createElement("a");
    a.href = resultUrl;
    a.download = `${base}-muted.${ext}`;
    a.click();
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <Breadcrumb locale={locale} homeLabel={tCommon("breadcrumbHome")} toolsLabel={tCommon("nav.tools")} toolLabel={t("label")} toolPath="mute-video" />
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
              ...
              {!videoFile ? (
                ...
                <div className="flex h-[200px] w-full items-center gap-4 rounded-xl border-2 border-dashed border-sky-500/50 bg-slate-900/60 p-4">
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-violet-500/20">
                    <VolumeX size={28} className="text-violet-400" />
                  </div>
                ...
                >
                  {isMuting ? (
                    <>
                      ...
                      <span>{t("mutingLabel")} {progress > 0 && `${progress}%`}</span>
                    </>
                  ) : (
                    <>
                      <VolumeX size={22} strokeWidth={2} className="shrink-0" />
                      <span>{t("muteButton")}</span>
                    </>
                  )}
                </button>
              )}
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.muteVideo" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="mute-video" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.muteVideo" />
        </div>
      </main>
    </div>
  );
