...
const AUDIO_ACCEPT = "audio/mpeg,audio/wav,audio/aac,audio/ogg,audio/x-m4a,audio/mp4,.mp3,.wav,.aac,.ogg,.m4a";
const AUDIO_TYPES = ["audio/mpeg", "audio/wav", "audio/aac", "audio/ogg", "audio/x-m4a", "audio/mp4"];
const EXT_MAP = {
  "audio/mpeg": "mp3",
  "audio/wav": "wav",
  "audio/aac": "aac",
...
function getExt(file) {
  const byType = EXT_MAP[file.type];
  if (byType) return byType;
  const match = file.name.match(/\.(mp3|wav|aac|ogg|m4a)$/i);
...
export default function TrimAudioPage() {
  ...
  const onAudioLoadedMetadata = useCallback(() => {
    ...
    if (a && isFinite(a.duration)) {
      ...
      setDuration(d);
      setEndSec((prev) => (prev === "" || Number(prev) > d ? String(d) : prev));
      setStartSec((prev) => (prev === "" || Number(prev) >= d ? "0" : prev));
    }
  }, []);
  ...
  const processFile = useCallback(
    (file) => {
      ...
      const ok = AUDIO_TYPES.includes(file.type) || /\.(mp3|wav|aac|ogg|m4a)$/i.test(file.name);
      if (!ok) {
  ...
  const handleDownload = useCallback(() => {
    ...
    const ext = getExt(audioFile);
    const base = audioFile?.name?.replace(/\.(mp3|wav|aac|ogg|m4a)$/i, "") || "trimmed";
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <Breadcrumb locale={locale} homeLabel={tCommon("breadcrumbHome")} toolsLabel={tCommon("nav.tools")} toolLabel={t("label")} toolPath="trim-audio" />
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.trimAudio" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="trim-audio" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.trimAudio" />
        </div>
      </main>
    </div>
  );
