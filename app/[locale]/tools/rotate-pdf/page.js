...
import { Upload, Loader2, Check, Download, RotateCw } from "lucide-react";
...
export default function RotatePdfPage() {
  ...
  const t = useTranslations("tools.rotatePdf");
  ...
  const handleRotate = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setError(t("errorRotateFailed"));
    } finally {
      setIsRotating(false);
    }
  }, [pdfFile, rotationAngle, t]);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          <section className="space-y-6">
            ...
            {pdfFile && !resultBlob && (
              <div className="space-y-3">
                <p className="text-sm font-medium text-slate-200">{t("rotationLabel")}</p>
                    ...
                    <button
                      ...
                      className={`rounded-xl border-2 px-4 py-2.5 text-sm font-semibold transition ${
                        rotationAngle === angle
                          ...
                          : "border-slate-600 bg-slate-800/60 text-slate-200 hover:border-sky-500/70"
                      }`}
                    >
                      {angle}°
                    </button>
              ...
              >
                {isRotating ? (
                  <>
                    ...
                    <span>{t("rotatingLabel")}</span>
                  </>
                ) : (
                  <>
                    <RotateCw size={22} strokeWidth={2} className="shrink-0" />
                    <span>{t("rotateButton")}</span>
                  </>
                )}
              </button>
            )}
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.rotatePdf" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="rotate-pdf" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.rotatePdf" />
        </div>
      </main>
    </div>
  );
