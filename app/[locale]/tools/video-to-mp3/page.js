...
const VIDEO_ACCEPT = "video/mp4,video/webm,video/quicktime,video/x-msvideo,video/x-matroska,.mp4,.webm,.mov,.avi,.mkv";
const INPUT_NAME = "input.mp4";
const OUTPUT_NAME = "output.mp3";

export default function VideoToMp3Page() {
  const locale = useLocale();
  const t = useTranslations("tools.videoToMp3");
  ...
  const getInputName = useCallback((file) => {
    const ext = (file.name.match(/\.(mp4|webm|mov|avi|mkv)$/i)?.[1] ?? "mp4").toLowerCase();
    return `input.${ext}`;
  }, []);
  ...
  const processFile = useCallback(
    (file) => {
      ...
      const ok = /\.(mp4|webm|mov|avi|mkv)$/i.test(file.name) || ["video/mp4", "video/webm", "video/quicktime", "video/x-msvideo", "video/x-matroska"].includes(file.type);
      if (!ok) {
  ...
  const handleExtract = useCallback(async () => {
    ...
    const inName = getInputName(videoFile);
    try {
      await ffmpeg.writeFile(inName, await fetchFile(videoFile));
      await ffmpeg.exec(["-i", inName, "-vn", "-acodec", "libmp3lame", "-q:a", "2", OUTPUT_NAME]);
  ...
  }, [videoFile, loadFfmpeg, getInputName, t]);

  const handleDownload = useCallback(() => {
    if (!resultBlob || !resultUrl) return;
    const base = videoFile?.name?.replace(/\.(mp4|webm|mov|avi|mkv)$/i, "") || "audio";
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <Breadcrumb locale={locale} homeLabel={tCommon("breadcrumbHome")} toolsLabel={tCommon("nav.tools")} toolLabel={t("label")} toolPath="video-to-mp3" />
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
                ...
                >
                  {isExtracting ? (
                    <>
                      ...
                      <span>{t("extractingLabel")} {progress > 0 && `${progress}%`}</span>
                    </>
                  ) : (
                    <>
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.videoToMp3" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="video-to-mp3" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.videoToMp3" />
        </div>
      </main>
    </div>
  );
