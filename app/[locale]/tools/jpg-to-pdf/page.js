...
export default function JpgToPdfPage() {
  ...
  const handleCreatePdf = useCallback(async () => {
    ...
    setIsCreating(true);
    try {
      const bytes = await jpgFile.arrayBuffer();
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
              ...
              {!jpgFile ? (
                ...
                <div className="flex h-[200px] w-full items-center gap-4 rounded-xl border-2 border-dashed border-sky-500/50 bg-slate-900/60 p-4">
                  <div className="flex h-full shrink-0 overflow-hidden rounded-lg bg-slate-800">
                    <img src={jpgUrl} alt="" className="h-full w-auto object-contain" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-slate-100">{jpgFile.name}</p>
                    <p className="mt-1 text-xs text-slate-400">{formatBytes(jpgFile.size)}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.jpgToPdf" />
          </aside>
        </div>
