...
const AUDIO_ACCEPT = "audio/wav,audio/ogg,audio/aac,audio/x-m4a,audio/mp4,audio/flac,audio/x-ms-wma,.wav,.ogg,.aac,.m4a,.flac,.wma";
const AUDIO_EXT_REGEX = /\.(wav|ogg|aac|m4a|flac|wma)$/i;
const AUDIO_TYPES = ["audio/wav", "audio/ogg", "audio/aac", "audio/x-m4a", "audio/mp4", "audio/flac", "audio/x-ms-wma"];
const OUTPUT_NAME = "output.mp3";

function getInputName(file) {
  const ext = (file.name.match(AUDIO_EXT_REGEX)?.[1] ?? "wav").toLowerCase();
  return `input.${ext}`;
}

export default function AudioToMp3Page() {
  const locale = useLocale();
  const t = useTranslations("tools.audioToMp3");
  ...
  const processFile = useCallback(
    (file) => {
      ...
      const ok = AUDIO_TYPES.includes(file.type) || AUDIO_EXT_REGEX.test(file.name);
      if (!ok) {
  ...
  const handleConvert = useCallback(async () => {
    ...
    const inName = getInputName(audioFile);
    try {
      await ffmpeg.writeFile(inName, await fetchFile(audioFile));
      await ffmpeg.exec(["-i", inName, "-acodec", "libmp3lame", "-q:a", "2", OUTPUT_NAME]);
  ...
  }, [audioFile, loadFfmpeg, t]);

  const handleDownload = useCallback(() => {
    if (!resultBlob || !resultUrl) return;
    const base = audioFile?.name?.replace(AUDIO_EXT_REGEX, "") || "audio";
    const a = document.createElement("a");
    a.href = resultUrl;
    a.download = `${base}.mp3`;
    a.click();
  }, [resultBlob, resultUrl, audioFile]);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <Breadcrumb locale={locale} homeLabel={tCommon("breadcrumbHome")} toolsLabel={tCommon("nav.tools")} toolLabel={t("label")} toolPath="audio-to-mp3" />
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          <section className="space-y-6">
            ...
            <div className="w-full">
              {!audioFile ? (
                ...
                >
                  <input ref={fileInputRef} type="file" accept={AUDIO_ACCEPT} className="sr-only" onChange={handleFileChange} />
              ...
              {audioFile && !resultBlob && !isLoadingFfmpeg && (
                <button
                  type="button"
                ...
                >
                  {isConverting ? (
                    ...
                    <>
                      <Music size={22} strokeWidth={2} className="shrink-0" />
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.audioToMp3" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="audio-to-mp3" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.audioToMp3" />
        </div>
      </main>
    </div>
  );
