...
import { Upload, Loader2, Check, Download, RectangleHorizontal } from "lucide-react";
...
const FORMAT_OPTIONS = [
  { id: "9:16", labelKey: "formatReels", w: 1080, h: 1920, blur: true },
  { id: "1:1", labelKey: "formatSquare", w: 1080, h: 1080, blur: true },
  { id: "16:9", labelKey: "formatWide", w: 1920, h: 1080, blur: false },
];
...
export default function ResizeVideoPage() {
  ...
  const t = useTranslations("tools.resizeVideo");
  ...
  const handleResize = useCallback(async () => {
    ...
    const opt = FORMAT_OPTIONS.find((o) => o.id === format) || FORMAT_OPTIONS[0];
    setError("");
    setResultBlob(null);
    setResultUrl((u) => {
      if (u) URL.revokeObjectURL(u);
    ...
    try {
      ...
      if (opt.blur) {
        const w = opt.w;
        const h = opt.h;
        const filter = `[0]split=2[main][copy];[copy]scale=${w}:${h}:force_original_aspect_ratio=increase,crop=${w}:${h},boxblur=20:5[bg];[main]scale=${w}:${h}:force_original_aspect_ratio=decrease,pad=${w}:${h}:(ow-iw)/2:(oh-ih)/2[fg];[bg][fg]overlay`;
        await ffmpeg.exec(["-i", inputName, "-filter_complex", filter, "-c:a", "copy", outputName]);
      } else {
        const vf = `scale=${opt.w}:${opt.h}:force_original_aspect_ratio=decrease,pad=${opt.w}:${opt.h}:(ow-iw)/2:(oh-ih)/2`;
  ...
  }, [videoFile, format, loadFfmpeg, t]);

  const handleDownload = useCallback(() => {
    if (!resultBlob || !resultUrl) return;
    const base = videoFile?.name?.replace(/\.(mp4|webm|mov)$/i, "") || "resized";
    const a = document.createElement("a");
    a.href = resultUrl;
    a.download = `${base}-${format.replace(":", "x")}.mp4`;
    a.click();
  }, [resultBlob, resultUrl, videoFile, format]);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <Breadcrumb locale={locale} homeLabel={tCommon("breadcrumbHome")} toolsLabel={tCommon("nav.tools")} toolLabel={t("label")} toolPath="resize-video" />
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
                ...
                <div className="flex h-[200px] w-full flex-col justify-center gap-4 rounded-xl border-2 border-dashed border-sky-500/50 bg-slate-900/60 p-4">
                  <div className="flex items-center gap-4">
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-violet-500/20">
                      <RectangleHorizontal size={28} className="text-violet-400" />
                    </div>
                    <p className="truncate text-sm font-medium text-slate-100">{videoFile.name}</p>
                  </div>
                  <div>
                    <p className="mb-2 text-xs font-medium text-slate-400">{t("formatSelectLabel")}</p>
                      ...
                      {FORMAT_OPTIONS.map((o) => (
                        <button
                          ...
                          className={`rounded-lg border px-4 py-2 text-sm font-medium transition ${
                            format === o.id ? "border-sky-500 bg-sky-500/20 text-sky-300" : "border-slate-600 bg-slate-800/60 text-slate-300 hover:border-slate-500"
                          }`}
                        >
                          {t(o.labelKey)}
                        </button>
              ...
              {videoFile && !resultBlob && !isLoadingFfmpeg && (
                ...
                >
                  {isProcessing ? (
                    ...
                    <>
                      <RectangleHorizontal size={22} strokeWidth={2} className="shrink-0" />
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.resizeVideo" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="resize-video" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.resizeVideo" />
        </div>
      </main>
    </div>
  );
