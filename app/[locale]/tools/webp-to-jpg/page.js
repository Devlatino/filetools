...
export default function WebpToJpgPage() {
  ...
  const t = useTranslations("tools.webpToJpg");
  ...
  const handleDownload = useCallback(() => {
    ...
    const base = originalFile?.name?.replace(/\.webp$/i, "") || "image";
    link.download = `fileflip-${base}.jpg`;
    document.body.appendChild(link);
    link.click();
    link.remove();
  }, [resultBlob, resultUrl, originalFile, t]);
      ...
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="space-y-8 lg:max-w-[340px]">
            <EditorialSection namespace="tools.webpToJpg" />
          </aside>
        </div>

        <div className="mt-10">
          <RelatedTools locale={locale} currentSlug="webp-to-jpg" />
        </div>

        <div className="mt-10">
          <FaqSection namespace="tools.webpToJpg" />
        </div>
      </main>
    </div>
  );
