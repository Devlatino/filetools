...
const BASE_URL = "https://fileflip.org";
const MAX_DESCRIPTION_LENGTH = 155;

/**
 * Build alternates.languages for a given path segment (e.g. "/tools/compress-image").
...
function buildLanguageAlternates(pathSegment) {
  ...
  languages["x-default"] = `/en${pathSegment}`;
  return languages;
}
...
export function buildToolMetadata({ locale, toolPath, title, description, keywords }) {
  ...
  const canonical = `${BASE_URL}/${locale}${pathSegment}`;
