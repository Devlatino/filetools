...
/**
 * Hook: init Three.js (npm), scene/camera/renderer in useEffect, then when geometryVersion > 0
 * apply geometry from geometryRef/boundsRef (positions, normals, bounds), center and scale to ~80%,
 * and attach orbit (rotate/zoom/pan). All THREE usage is inside useEffect so it runs in browser.
 */
export function useThreeViewer(containerRef, geometryRef, boundsRef, geometryVersion) {
  ...
  useEffect(() => {
    ...
    if (!geometry || !bounds || !scene || !camera) return;
    ...
    const pos = new THREE.BufferAttribute(geometry.positions, 3);
    const norm = new THREE.BufferAttribute(geometry.normals, 3);
    const geom = new THREE.BufferGeometry();
    geom.setAttribute("position", pos);
    geom.setAttribute("normal", norm);
    const mat = new THREE.MeshStandardMaterial({ color: VIEWER_MESH_COLOR });
  ...
  const attachOrbit = useCallback(() => {
    ...
    const onMouseMove = (e) => {
      ...
      if (orbit.isPanning) {
        const forward = new THREE.Vector3(orbit.target[0], orbit.target[1], orbit.target[2]).sub(camera.position).normalize();
